...
function buildQuickReplies(params: {
  ...
  chips.push({ label: 'Help me choose', action: 'Suggest the best compatible meal bundle' })
...
function scoreMenuItem(item: MenuItemAIContext, message: string): number {
  ...
  // revenue bias: slightly prefer stronger upsell items when compatible
...
function buildFallbackReply(
  ...
  isRiceQuery(message) ||
  /best seller|chef special|veg|non-veg/i.test(message)

if (
  stage === 'early' &&
  preference === 'unknown' &&
  hasNonVeg &&
  isBroadChoiceQuery(message) &&
  !isChipQuery
) {
    return {
  ...
  const upsell = chooseUpsells(primary, menuItems, message)

  return {
    reply: composeReply(primary, upsell, message),
...
function choosePrimaryItem(
  ...
  const scored = candidates
    .map((item) => {
      ...
      // Better anchor items for broad queries
      ...
      // Prefer dishes with explicit pair info
      if ((item.best_with?.length ?? 0) > 0) score += 3

      // Prefer bestseller only when still relevant
...
function compatibilityHintsForPrimary(primary: MenuItemAIContext) {
  ...
  for (const rule of REVENUE_PAIR_RULES) {
    if (rule.primaryPatterns.some((rx) => rx.test(combined))) {
      return rule
    }
  }
...
function chooseUpsells(primary: MenuItemAIContext | undefined, items: MenuItemAIContext[], message: string) {
  ...
  // If user explicitly asked for dessert, keep the upsell tight and relevant.
  if (isDessertQuery(message)) {
  ...
  // If user asked for a drink only, keep it tight too.
  ...
  // Bread query: bread + one strong complement.
  ...
  const wantsDessert = isDessertQuery(message)
  const wantsDrink = isDrinkQuery(message)
...
function composeReply(primary: MenuItemAIContext | undefined, upsell: string[], message: string) {
  if (!primary) {
    return 'I can help you choose from the menu. Tell me if you want veg, non-veg, spicy, or a full meal, and I will suggest the best items.'
  }

  const reason = getReasonText(message)
  const descText = primary.description?.trim() ? ` ${primary.description.trim().replace(/\.$/, '')}.` : ''
  const pairText = upsell.length ? ` Pair it with ${buildNaturalList(upsell)}.` : ''

  return sanitizeReply(`${primary.name} is a strong choice ${reason}.${descText}${pairText}`)
}
...
function parseStructuredReply(
  ...
  if (!parsed) {
    return buildFallbackReply(message, stage, menuItems, preference, hasNonVeg)
  }
  ...
  const primary = mentioned_items.length
    ? menuItems.find((i) => i.name === mentioned_items[0])
    : choosePrimaryItem(message, menuItems, preference)

  const finalUpsell =
    upsell_items.length > 0 ? upsell_items : primary ? chooseUpsells(primary, menuItems, message) : []

  return {
    reply: reply || composeReply(primary, finalUpsell, message),
    mentioned_items: mentioned_items.length ? mentioned_items : primary ? [primary.name] : [],
    upsell_items: finalUpsell.slice(0, 3),
    psych_trigger: inferPsychTrigger(message, primary, finalUpsell),
    convo_stage: (typeof parsed.convo_stage === 'string' ? (parsed.convo_stage as ConvoStage) : stage) ?? stage,
  }
}
...
function buildSystemPrompt(restaurantName: string, ctx: MenuContextPayload, message: string): string {
  ...
  const pairingsPreview = relevantItems.flatMap((item) => chooseUpsells(item, menuItems, message)).slice(0, 6)
...
You must be precise, useful, and menu-aware.
Do NOT act like a generic chatbot.
Do NOT recommend items unless they match the user's request.
Do NOT choose weak pairings when a better compatible pairing exists.
...
- If the user asks for filling food, choose a hearty main and pair it with bread first, then rice or dessert only if it truly fits.
- If the user asks for dessert, recommend a dessert from the menu and optionally pair it with tea, coffee, or another dessert if available.
- If the user names a dish like Paneer Butter Masala, use the dish itself as the anchor and suggest the strongest compatible complements such as Garlic Butter Naan, Butter Naan, or Tandoori Roti if they exist.
- If the user names Jeera Rice, suggest Dal Tadka / Raita / dessert before random bread.
- If the user wants vegetarian food, only use veg items.
- If the user asks for dessert, ONLY recommend dessert items and optionally one drink (tea/coffee/lassi). Never add rice, bread, or mains to a dessert query.
- If only one dessert exists on the menu, return only that one item. Never pad upsells with unrelated items just to fill slots.
- If the user asks for a specific course (bread, drink, starter), only return items from that course plus the single strongest complement. Never return items from 3+ different courses for a single-course query.
...
- Explain WHY the pairing is good.
- Optimize for a natural, revenue-smart basket, not just item count.
- Prefer the strongest compatible upsell over generic filler items.
...
function getReasonTextFromPrimary(primary: MenuItemAIContext | undefined) {
  if (!primary) return 'for this order'
  const course = getCourseGroup(primary)
  if (course === 'main') return 'for a richer meal'
  if (course === 'rice') return 'to complete the plate'
  if (course === 'bread') return 'to make the curry shine'
  if (course === 'dessert') return 'to finish the meal'
  if (course === 'starter') return 'to start strong'
  return 'for this order'
}

function inferReplyFromSelections(primary: MenuItemAIContext | undefined, upsell: string[]) {
  if (!primary) {
    return 'I can help you choose from the menu. Tell me what mood you are in, and I will suggest the best dishes.'
  }

  const reason = getReasonTextFromPrimary(primary)
  const pairText = upsell.length ? ` Pair it with ${buildNaturalList(upsell)}.` : ''
  const description = primary.description?.trim()
  const descText = description ? ` ${description.replace(/\.$/, '')}.` : ''
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Deterministic smart waiter selection first
    const primary = choosePrimaryItem(message, menuItems, preference)
const rawUpsell = chooseUpsells(primary, menuItems, message)
// Never return more upsell items than actually exist matching the intent
const upsell = rawUpsell.filter(name => menuItems.some(i => i.name === name))
const reply = inferReplyFromSelections(primary, upsell)
    const suggestions = buildQuickReplies({ hasNonVeg, preference, menuItems })
