...
async function parseMenuWithGemini(base64Data: string, mimeType: string): Promise<GeminiMenuResult> {
  ...
  if (start === -1 || end === -1) throw new Error('Could not find JSON in Gemini response. Try a clearer image.')
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5">
          ...
          {step === 'choose' && (
            <div className="space-y-3">
              ...
              >
                ...
                <div>
                  <p className="text-sm font-semibold text-white">Scan Photo</p>
                  <p className="text-xs text-zinc-500">Take a photo of your physical menu</p>
                </div>
              </button>

              <button
                onClick={() => fileRef.current?.click()}
                className="flex w-full items-center gap-4 rounded-2xl border border-zinc-700/60 bg-zinc-800/40 p-4 text-left transition hover:border-amber-500/50 hover:bg-zinc-800/80"
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500">PDF, JPG, PNG, WEBP</p>
                </div>
              </button>
          ...
          {step === 'importing' && (
            <div className="py-10 text-center">
              ...
              <p className="mt-1 text-xs text-zinc-500">This will only take a moment</p>
            </div>
          )}
...
>('premium')
  ...
  async function addCategory(name?: string) {
    ...
    try {
      ...
      if (data) {
        setCategories((prev) => [...prev, data as MenuCategoryRow])
        setActiveCat((data as MenuCategoryRow).id)
        setNewCatName('')
      }
    } catch (err) {
            ...
            {activeCat ? (
              <>
                <div className="mb-4 flex items-center justify-between gap-3">
                  <div>
                    <p className="text-lg font-bold text-white">{activeCatData?.name}</p>
        ...
        {editingItem.description?.trim() ? 'Improve with AI' : 'Generate with AI'}
                ...
                <div className="space-y-2">
                  <ToggleRow
                    ...
                    checked={Boolean(editingItem.is_bestseller)}
                    onChange={(checked) => setEditingItem((f) => (f ? { ...f, is_bestseller: checked } : f))}
                  />
                  <ToggleRow
                    label="Available"
                    description="Show to customers"
                    checked={Boolean(editingItem.is_available)}
                    onChange={(checked) => setEditingItem((f) => (f ? { ...f, is_available: checked } : f))}
                  />
                </div>
