...
export function VerifyVisitCard({ restaurantId }: Props) {
  ...
  const handleVerify = useCallback(async () => {
    ...
    if (clean.length !== 4) {
      setResult({ ok: false, message: 'Enter the 4-digit PIN shown by the guest.' })
      return
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setResult({ ok: true, message: `+${json.points_awarded} points awarded to guest!` })
        setPin('')
      }
    } catch {
  ...
  return (
    <div className="rounded-2xl border border-white/[0.06] bg-[#111111] p-5 sm:p-6">
      ...
      <p className="mb-3 text-[11px] leading-relaxed text-zinc-600">
        Ask the guest for their 4-digit PIN after the meal and enter it here to award 50 loyalty points.
      </p>
